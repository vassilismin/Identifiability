...
profile_likelihood <- function(X){
  ...
  N_thetas=X$N_thetas # The number of samples for each parameter
  ...
  # Forward search of theta_i neighborhood 
  theta_i_step <- seq(theta_i_optimal, ub[i], length.out=N_thetas/2)[-1] 
  ...
  forward_results_df <- data.frame(matrix(NA, nrow=length(theta_i_step), ncol = 2))
  ...
  for (j in 1:length(theta_i_step)) { # loop over every instance of theta_i_step
    ...
    forward_results_df[j,] <- c(constant_theta, optimization$objective)
    
    # Check if the x^2 value exceeded the threshold to stop the iterations
    if(optimization$objective >= global_optimum + Delta_alpha) break
  }
  cat("Forward search ended after ", j, "iterations.", "\n")
  
  cat("Backward search begins...", "\n")
  # Backwards search of theta_i neighborhood 
  theta_i_step <- seq(theta_i_optimal, lb[i], length.out=N_thetas/2)[-1] 
  # For the i-th key create a 2-columns dataframe to store the tested theta_i values
  # and the corresponding values of the objective function
  backward_results_df <- data.frame(matrix(NA, nrow=length(theta_i_step), ncol = 2))
  ...
  for (j in 1:length(theta_i_step)) { # loop over every instance of theta_i_step
    ...
    backward_results_df[j,] <- c(constant_theta, optimization$objective)
    
    # Check if the x^2 value exceeded the threshold to stop the iterations
    if(optimization$objective >= global_optimum + Delta_alpha) break
  }
  cat("Backward search ended after ", j, "iterations.", "\n")
...
for(i in 1:5){
  X[[i]] <- list(index=i, thetas=thetas, thetas_names=thetas_names,
                 lb=lb, ub=ub, N_thetas=100, N_iter=500,
                 alpha=0.95, df=1,
...
# legend_plot <- ggplot() +
#   geom_line(aes(c(1,2), c(1,2), color = "chi^2"), size = 2) +
#   geom_hline(aes(yintercept = 2, linetype = "95% Confidence intervals"), color = "red", size = 1) +
#   geom_hline(aes(yintercept = 3, linetype = "Global optimal value"), color = "blue", size = 1) +
#   scale_color_manual(
#     values = c("black", "red", "blue"),
#     labels = c(expression(paste(chi^2, "(", theta, ")")), "95% Confidence intervals", "Global optimal value")
#   ) +
...
# Combine the plots and legend using patchwork
# combined_plot <- wrap_plots(plot_list) +
#   plot_layout(nrow = 3) +
#   plot_annotation(legend_plot, width = unit(0.2, "npc"), height = unit(0.2, "npc"),
#                   top = unit(0.8, "npc"), right = unit(0.8, "npc"))
